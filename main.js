...
app.whenReady().then(() => {
  ...
  ipcMain.handle('refresh-google-token', async (event, refreshToken) => {
    const CLIENT_ID = process.env.GOOGLE_CLIENT_ID;
    const CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET;

    try {
      console.log("Main Process: Requesting new access token from Google...");
      const response = await fetch('https://oauth2.googleapis.com/token', {
        method: 'POST',
        headers: {'Content-Type' : 'application/x-www-form-urlencoded'},
        body: new URLSearchParams({
      ...
      console.log("Main Process: Successfully generated new access token!");
      return tokens;
    } catch (error) {
      console.log("Main Process: Token refresh failed:", error);
      throw error;
    }
  });
