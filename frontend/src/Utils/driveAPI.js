...
export const initializeDriveFolder = async () => {
    ...
    const searchResponse = await fetch(`${DRIVE_API_URL}?q=${query}&fields=files(id, name)`, {
        method: 'GET',
...
export const checkLockStatus = async (projectFolderId) => {
    ...
    const contentResponse = await fetch(`${DRIVE_API_URL}/${lockFileId}?alt=media`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
...
export const acquireLock = async (projectFolderId, nickName) => {
    ...
    const updateResponse = await fetch(`https://www.googleapis.com/upload/drive/v3/files/${newLockFileId}?uploadType=media`,{
        method: 'PATCH',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
export const releaseLock = async (lockFileId) => {
    ...
    const response = await fetch(`${DRIVE_API_URL}/${lockFileId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
    });
...
export const uploadProjectDataToDrive = async (projectFolderId, projectData) => {
    ...
    const searchResponse = await fetch(`${DRIVE_API_URL}?q=${query}&fields=files(id)`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
...
export const downloadProjectDataFromDrive = async (projectFolderId) => {
    ...
    const contentResponse = await fetch(`${DRIVE_API_URL}/${fileId}?alt=media`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
